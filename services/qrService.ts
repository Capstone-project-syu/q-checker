...
const createQR = async (event: EventRequest) => {
    try {
        ...
        Alert.alert("QR 생성 실패", "QR 코드를 생성하는 중 문제가 발생했습니다. 다시 시도해주세요.");
    }
}
...
const getEvent = async () => {
    try {
        ...
        Alert.alert("Event 조회 실패", "Event를 조회하는 중 문제가 발생했습니다. 다시 시도해주세요.");
    }
}
...
const getQR = async (qrcodeId: number) => {
    try {
        ...
        Alert.alert("QR 조회 실패", "QR 코드를 조회하는 중 문제가 발생했습니다. 다시 시도해주세요.");
    }
}

const getAttendance = async (eventId: number) => {
    try {
        const res = await axios.get(`https://api.qchecker.me/attendances/events/${eventId}/`);
        console.log("Attendance 조회 성공:", res.data);
        return res.data;
    } catch (e) {
        const errorRes = (e as AxiosError).response;
        console.error("Attendance 조회 실패:", e);
        Alert.alert("Attendance 조회 실패", "Attendance를 조회하는 중 문제가 발생했습니다. 다시 시도해주세요.");
    }
}
